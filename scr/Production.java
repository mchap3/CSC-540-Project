...
public class Production {
	...
	public void createBook() {
		try {
			...
			String sql4 = String.format("select * from Publication natural join Books where PublicationID = %d;", pubID);
			String sql5 = String.format("select Name as 'Author(s)' from WritesBook natural join Employees where PublicationID = %d;", pubID);
	...
	public void editBook() {
		try {
			...
			if (scanner.nextLine().toLowerCase().startsWith("y")) {
				int empID;
				while (true) {
					System.out.print("1. Remove Author\n"
							+ "2. Add author\n"
					...
					if (input.equals("1")) {
						...
						while (true) {
							...
							if (input.toLowerCase().equals("new")) {
								...
								empID = Integer.valueOf(input);
							String sql8 = String.format("insert into WritesBook values(%d, %d);", newPubID == 0 ? pubID : newPubID, empID);
							db.update(sql8);
						}
			...
			String sql9 = String.format("select * from Publication natural join Books where PublicationID = %d;", newPubID == 0 ? pubID : newPubID);
			String sql10 = String.format("select Name as 'Author(s)' from WritesBook natural join Employees where PublicationID = %d;", newPubID == 0 ? pubID : newPubID);
	...
	public void deleteBook() {
		try {
			...
			// get book information
			String sql2 = String.format("select * from Publication natural join Books where PublicationID = %d;", pubID);
	...
	public void addChapter() {
		try {
			...
			String sql2 = String.format("select * from Chapters where PublicationID = %d and ChapterTitle = '%s';", pubID, chTitle);
			result = db.query(sql2);
			DBTablePrinter.printResultSet(result);
	...
	public void editChapter() {
		try {
			...
			String sql2 = String.format("update Chapters set ChapterTitle = '%s' where PublicationID = %d and ChapterTitle = '%s';", newChTitle, pubID, chTitle);
			db.update(sql2);
			
			// confirmation
			System.out.println("Successfully Updated Following Record:");
			String sql3 = String.format("select * from Chapters where PublicationID = %d and ChapterTitle = '%s';", pubID, newChTitle);
			...
			if (scanner.nextLine().toLowerCase().startsWith("y")) {
				...
				while (true) {
					...
					if (input.toLowerCase().equals("new")) {
						...
						empID = Integer.valueOf(input);
					String sql2 = String.format("insert into WritesArticle values(%d, '%s', %d);", pubID, articleTitle, empID);
					db.update(sql2);
				}
			}
			
			// confirmation
			String sql3 = String.format("select * from Articles where PublicationID = %d and ArticleTitle = '%s';", pubID, articleTitle);
			String sql4 = String.format("select Name as 'Author(s)' from WritesArticle natural join Employees where PublicationID = %d and ArticleTitle = '%s';", pubID, articleTitle);
			result = db.query(sql3);
	...
	public void updatePayment() {
		try {
			...
			int checkNum = scanner.nextInt(); scanner.nextLine();
//			System.out.print("Enter Employee ID number: ");
//			int empID = scanner.nextInt(); scanner.nextLine();
			...
			String sql = String.format("update Payments set ClaimDate = '%s' where CheckNumber = %d;", claimDate, checkNum);
			db.update(sql);
